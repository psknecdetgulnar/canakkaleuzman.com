    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="mt-1 text-sm text-[#102844]">
          {isPremium
            ? "Profilindeki randevu formundan gelen talepler burada listelenir. Talep geldiğinde ziyaretçiyle doğrudan telefon/WhatsApp üzerinden de iletişime geçebilirsin."
            : "Free profilde yalnızca bir talep geldiğini bildirim olarak görürsün — ziyaretçinin adı, telefonu ve notu gizli kalır. Detayları görüp iletişime geçmek için premium'a yükselt."}
        </p>
      </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          {items.map((a) =>
            isPremium ? (
              ...
              // Free profil: yalnızca bir talep geldiği bildirilir — ziyaretçinin
              // kimliği, telefonu ve notu (dönüşüm bilgisi) gizli kalır.
              <div
                key={a.id}
                className="flex flex-wrap items-center justify-between gap-3 rounded-[14px] border border-dashed border-[rgba(16,40,68,0.18)] bg-[#f3eee6] p-5"
              >
                <div className="flex items-center gap-3">
                  <span className="text-lg" aria-hidden="true">🔒</span>
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-[#0d2c4b]">Yeni randevu talebi</span>
                      <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${STATUS_STYLE[a.status]}`}>
                        {STATUS_LABEL[a.status]}
                      </span>
                    </div>
                    <p className="mt-1 text-xs text-[rgba(16,40,68,0.6)]">
                      Ziyaretçi adı, telefonu ve notu premium'da görünür · {formatDate(a.createdAt)}
                    </p>
                  </div>
                </div>
              </div>
          ...
          {!isPremium && items.length > 0 && (
            <button
              type="button"
              className="self-start rounded-[6px] bg-[#c99a53] px-4 py-2 text-sm font-semibold text-[#fffdf9] transition-colors hover:bg-[#b98742]"
